  ...
  return (
    ...
    <div className='flex justify-between items-center px-4 bg-white w-screen h-[56px]'>
          ...
          <div className="center flex h-full ">
            <div  className='relative flex justify-center items-center  w-[111px] h-full cursor-pointer' onClick={()=>{setActiveLink('/')}}>   <GoHomeFill className={`w-[30px] h-[30px] text-gray-400 cursor-pointer transition-all duration-300 ${activeLink == '/' ? 'text-blue-500' : ''}`}/>               <span className={`absolute bg-blue-500 w-full h-1 rounded-full -bottom-1 opacity-0 transform transition-all duration-300 ${activeLink === '/' ? 'bottom-0 opacity-100' : ''} `}/>  </div>  
            <div className='relative flex justify-center items-center  w-[111px] h-full cursor-pointer' onClick={()=>{setActiveLink('/Friends')}}> <FaUserFriends className={`w-[30px] h-[30px] text-gray-400 cursor-pointer transition-all duration-300 ${activeLink == '/Friends' ? 'text-blue-500' : ''}`}/> <span className={`absolute bg-blue-500 w-full h-1 rounded-full -bottom-1 opacity-0 transform transition-all duration-300 ${activeLink === '/Friends' ? 'bottom-0 opacity-100' : ''} `}/> </div> 
            <div className='relative flex justify-center items-center  w-[111px] h-full cursor-pointer' onClick={()=>{setActiveLink('/Groups')}}> <HiMiniUserGroup className={`w-[30px] h-[30px] text-gray-400 cursor-pointer transition-all duration-300 ${activeLink == '/Groups' ? 'text-blue-500' : ''}`}/> <span className={`absolute bg-blue-500 w-full h-1 rounded-full -bottom-1 opacity-0 transform transition-all duration-300 ${activeLink === '/Groups' ? 'bottom-0 opacity-100' : ''} `}/> </div> 
          </div>
