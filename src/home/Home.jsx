...
import { FaCirclePlus, FaVirusCovid } from 'react-icons/fa6'
  ...
  return (
    ...
    <>
      ...
      <div className={`bg-[#f0f2f5] w-screen  h-[720px] py- flex justify-between`}>
        ...
        <div className="center xl:w-[54%]  xl:px-16 bg-white rounded-lg border flex flex-col  gap-4 items-center xl:pt-4 overflow-y-auto no-scrollbar">
          ...
          <div className="createPost w-full rounded-lg p-3 flex xl:flex-col justify-between gap-3 border" onClick={(e) => { handleButtonClick(e, createPostPopup) }}>
            ...
            <div className="upper flex items-center w-full gap-3 justify-between ">
              ...
              <div className="inputDiv border rounded-full flex-grow  flex pl-3 items-center h-[40px] bg-gray-100 text-gray-400 hover:bg-gray-200 cursor-pointer xl:text-base text-sm">What's on your mind, Bhushan</div>
            </div>

            <div className='border-b hidden xl:block' />

            <div className="lower flex justify-between ">
              <div className="liveVideo hidden xl:flex gap-2 items-center justify-center w-full py-1 hover:bg-gray-100 rounded-lg cursor-pointer"> <img src={liveVideoIcon} alt="" /> <span>Live video</span> </div>
              <div className='photo/video flex xl:flex-row flex-col gap-2 items-center justify-center w-full py-1 hover:bg-gray-100 rounded-lg cursor-pointer'> <img src={photosVideosIcon} alt="" /> <span className='xl:text-base text-xs'>Photos</span> </div>
              <div className='feeling/activity hidden xl:flex  gap-2 items-center justify-center w-full py-1 hover:bg-gray-100 rounded-lg cursor-pointer'> <img src={feelingActivityIcon} alt="" /> <span>Feelings</span> </div>
            </div>

          </div>


          <div ref={createPostPopup} className={`w-screen h-screen flex justify-center items-center fixed left-0 top-0 backdrop-blur-[2px] ${child === createPostPopup && active ? 'showFromTop z-50' : 'hideFromBottom'}`} onClick={(e) => { createPostContainer.current.contains(e.target) ? '' : setChild(null) }}>

            <div ref={createPostContainer} className={`createComment bg-white  absolute z-50 flex flex-col justify-between gap-3 p-3  xl:left-[23.3%] xl:bottom-[5%] rounded-lg xl:w-[820px] xl:h-[670px] h-[500px] boxShadow `}>

              <div className="heading p-1 xl:p-3 border rounded-lg bg-slate-100 flex justify-center xl:text-lg tracking-wider font-bold"> Create post </div>
        ...
        <div className="right hidden xl:flex flex-col w-[24%] bg-white rounded-lg border gap-4  overflow-y-auto no-scrollbar p-4">
