...
function Post({ postInfo, setPostDeleted, setPostCommentCount }) {
  ...
  return (
    ...
    <>
      ...
      <div className='MOBILE xl:hidden relative w-full rounded-lg flex flex-col gap-3 py-4   '>
        ...
        <div className='UserInfo flex items-center gap-3 px-4 '>
          ...
          <div className='channalName&PostInfo flex flex-col justify-center leading-5 h-[40px] '>
            <div className="channalInfo flex items-center gap-2 "> <span className='font-semibold' onClick={() => { unfollowUser(postInfo.author._id) }}>{postInfo.author.name}</span> <div className='w-1 h-1 bg-gray-500 rounded-full' /> {followedFriends && followedFriends.includes(postInfo.author._id) ? <span className='font-semibold text-blue-500' onClick={() => { unfollowUser(postInfo.author._id) }}>Unfollow</span> : <span className='font-semibold text-blue-500' onClick={() => { followUser(postInfo.author._id) }}>Follow</span>}  </div>
            <div className="postInfo flex items-center gap-2 font-semibold text-gray-400"> <span className='text-xs'>Recomended post</span> <div className='w-1 h-1 bg-gray-500 rounded-full' /> <span className='text-xs '>{years > 0 ? `${years} years` : ``} {months > 0 ? `${months} months` : ``} {days > 0 ? `${days} days` : ``} {years == 0 && months == 0 && days == 0 ? `${hours} hours` : ``} ago</span> <div className='w-1 h-1 bg-gray-500 rounded-full' /> <span className='text-xs'>public</span> </div>
          </div>
        </div>
        ...
        <div className='Like Comment Share Buttons relative flex justify-between px-4 text-xs font-semibold text-gray-500'>

          <div className="like peer flex gap-2 items-center cursor-pointer "><img className='w-[15px] h-[15px]' src={likeblankicon} alt="" /> <span>Like</span> </div>
          <div className="reactions bg-white px-4 gap-3 h-[45px] rounded-full flex justify-between absolute border transition-all duration-300 scale-0 -translate-x-[90px] -translate-y-6 peer-hover:-translate-y-[25px] peer-hover:translate-x-1 peer-hover:scale-100 hover:-translate-y-[25px] hover:translate-x-1 hover:scale-100"> <img className='cursor-pointer transition-all duration-300 hover:scale-[1.2] hover:-translate-y-2' src={likegif} alt="" onClick={() => { likePost() }} /> <img className='cursor-pointer transition-all duration-300 hover:scale-[1.2] hover:-translate-y-2' src={dislikegif} alt="" onClick={() => { dislikePost() }} /> <img className='cursor-pointer transition-all duration-300 hover:scale-[1.2] hover:-translate-y-2' src={wowGif} alt="" /> <img className='cursor-pointer transition-all duration-300 hover:scale-[1.2] hover:-translate-y-2' src={laughgif} alt="" />  <img className='cursor-pointer transition-all duration-300 hover:scale-[1.2] hover:-translate-y-2' src={caregif} alt="" /></div>

          <div className="comment flex gap-2 items-center cursor-pointer" onClick={(e) => { handleButtonClick(e, commentPopup); getPostComments() }}><img className='w-[15px] h-[15px]' src={commentblankicon} alt="" /> <span>Comment</span></div>

          <div ref={commentPopup} className={`w-screen h-screen fixed left-0 top-0 backdrop-blur-[2px] ${child === commentPopup && active ? 'showFromTop z-50' : 'hideFromBottom'}`} onClick={(e) => { commentContainer.current.contains(e.target) ? '' : setChild(null) }}>

            <div ref={commentContainer} className={`createComment bg-white  absolute z-50 flex flex-col  justify-between gap-3 p-3  left-0 top-16 rounded-lg w-full h-[70%] boxShadow `}>
              <div className="upper relative flex justify-center items-center  w-full  text-base font-bold tracking-wide ">Bhushan's Post  <MdClose className='absolute right-2 top-2 cursor-pointer ' onClick={() => { setChild(null) }} /></div>
              ...
              <div ref={middleDivRef} id='middle' className="middle flex flex-col gap-3 pt-3 overflow-y-auto no-scrollbar w-full h-[74%] ">
                <div className='UserInfo flex items-center gap-3 px-4'>
                  ...
                  <div className='channalName&PostInfo flex flex-col justify-center leading-5 h-[40px] '>
                    <div className="channalInfo flex items-center gap-2 ">
                      <span className='font-semibold text-base'>{postInfo.author.name}</span>
                      <div className='w-1 h-1 bg-gray-500 rounded-full' />
                      <span className='font-semibold text-blue-500'>Follow</span>
                    </div>
                    <div className="postInfo flex items-center gap-2 text-xs text-gray-400 ">
                      <span className=''>Recomended post</span>
                      <div className='w-1 h-1 bg-gray-400 rounded-full' />
                      <span className=''>{years > 0 ? `${years} years` : ``} {months > 0 ? `${months} months` : ``} {days > 0 ? `${days} days` : ``} {years == 0 && months == 0 && days == 0 ? `${hours} hours` : ``} ago</span>
                      <div className='w-1 h-1 bg-gray-400 rounded-full' />
                ...
                <div className='WhatsOnUsersMind px-2 leading-5 text-[14px]'> {postInfo.content} </div>
          ...
          <div className="send flex gap-2 items-center"><img className='w-[15px] h-[15px]' src={whatsappblankicon} alt="" /> <span>Send</span></div>

          <div className="share flex gap-2 items-center"><img className='w-[15px] h-[15px]' src={shareblankicon} alt="" /> <span>Share</span></div>
        </div>
