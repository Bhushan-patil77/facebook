...
function Usercard({postInfo}) {
  return (
    <div className='w-[230px] h-[385px] rounded-md flex flex-col justify-between bg-white shadow'>
      <div className="imageAndName flex flex-col ">
        <div className='w-full h-[230px] rounded-md bg-gray-50'>{postInfo.author.profileImage != null ? <img className='w-full rounded-t-md' src={postInfo.author.profileImage} alt="" /> : <FaUserLarge className='w-full h-full text-gray-100 p-2'/> }</div>
        <span className='font-semibold p-2 capitalize'>{postInfo.author.name}</span>
      </div>
